...
namespace CW.NETMF.Hardware
{
  ...
  #pragma warning restore 649
    ...
    /// </summary>
    /// <returns>
    /// The bit read. If there is no device on the bus the return value is <b>true</b>.
    /// </returns>
    /// <exception cref="ObjectDisposedException">
    /// The method was called after the object was disposed.
    /// </exception>
    [MethodImpl(MethodImplOptions.InternalCall)]
    public extern bool ReadBit();
    ...
    /// </remarks>
    public int Search(byte[] pattern, int deviation, int index = 0)
    {
      return Search(0xF0, pattern, deviation, index);
    }

    public int AlarmSearch(byte[] pattern, int deviation, int index = 0)
    {
      return Search(0xEC, pattern, deviation, index);
    }

    [MethodImpl(MethodImplOptions.InternalCall)]
    protected extern int Search(byte command, byte[] pattern, int deviation, int index);
