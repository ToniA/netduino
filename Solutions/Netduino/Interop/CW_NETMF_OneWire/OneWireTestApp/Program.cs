...
namespace OneWireTestApp
{
  ...
  public class Program
  {
    ...
    private static void sw1_OnInterrupt(uint portId, uint state, DateTime time)
    {
      ...
      if(oneWire.Reset())
      {
        oneWire.Write(OneWireCommand.ReadRom);
        oneWire.Read(rom);
      ...
      do
      {
        if((deviation = oneWire.Search(rom, deviation)) < 0)
          break;
        // The CRC is checked in Search() method
        //if(OneWire.ComputeCRC(rom, count:7) == rom[7])
        {
      ...
      if(oneWire.Search(foundRom, 64) == 0)
      {
        ...
        if(rom[7] == foundRom[7]) // Comparing CRC is enough for test purposes
        {
          ...
          if(rom[0] == DS18B20.FamilyCode)
          {
            ...
            oneWire.Write(DS18B20.ReadScratchpad);
      ...
      {
        oneWire.Reset();
        oneWire.Write(OneWireCommand.SkipRom); // Address all devices
        oneWire.Write(DS18B20.ConvertT);
        ...
        matchRom[0] = OneWireCommand.MatchRom;
        ...
        oneWire.Reset();
        oneWire.Write(matchRom);
        oneWire.Write(DS18B20.ReadPowerSupply);
        if(!oneWire.ReadBit())
        {
