...
:: GCC version number for comparison in .targets file (4.7.3 -> 40703)
for /f "tokens=1 delims=." %%i in ('echo %GNU_VERSION%') do set /a COMPILER_TOOL_VERSION_MAJ=%%i
for /f "tokens=2 delims=." %%i in ('echo %GNU_VERSION%') do set /a COMPILER_TOOL_VERSION_MIN=%%i
for /f "tokens=3 delims=." %%i in ('echo %GNU_VERSION%') do set /a COMPILER_TOOL_VERSION_REV=%%i
set /a COMPILER_TOOL_VERSION_INT=%COMPILER_TOOL_VERSION_MAJ%*10000+%COMPILER_TOOL_VERSION_MIN%*100+COMPILER_TOOL_VERSION_REV

SET COMPILER_TOOL_VERSION_NUM=%COMPILER_TOOL_VERSION_MAJ%.%COMPILER_TOOL_VERSION_MIN%
SET COMPILER_TOOL_VERSION=%COMPILER_TOOL%%COMPILER_TOOL_VERSION_NUM%
SET DOTNETMF_COMPILER=%COMPILER_TOOL_VERSION%
